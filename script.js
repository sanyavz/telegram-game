...
const items = [];
const itemImages = ['item1.png', 'item2.png', 'item3.png', 'itemBad.png'].map(src => { 
  let img = new Image(); 
  img.src = 'assets/' + src; 
  return img; 
});
...
function spawnItem() {
  ...
  const baseSize = 50;
  const size = type === 3 ? baseSize * 4 : baseSize; // плохой предмет в 4 раза больше
...
function update() {
  ...
  for (let i = 0; i < items.length; i++) {
    items[i].y += 5 / 3; // замедление падения в 3 раза
